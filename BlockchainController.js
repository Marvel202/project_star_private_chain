...
class BlockchainController {
    ...
    submitStar() {
        this.app.post("/submitstar", async (req, res) => {
            if(req.body.address && req.body.message && req.body.signature && req.body.star) {
                ...
                console.log("----",star, signature, message, address)
                try {
    ...
    getAllBlocks() { this.app.get("/allblocks", async (req, res) =>
    { 
        
        // let errorList = await this.blockchain.validateChain();
        let chain = await this.blockchain.chain;
         if(chain) 
            { 
                return res.status(200).json(chain); 
            } 
        else { return res.status(404).send("chain not found"); 
        }
     }); 
    }

    getchainValidate() {this.app.get("/validateChain", async (req, res) => {
        let errList = await this.blockchain.validateChain();
        if(errList.length > 0) {
            return res.status(500).json(errList)
        } else {
            return res.status(202).send("chain validated!")
        }
    })
